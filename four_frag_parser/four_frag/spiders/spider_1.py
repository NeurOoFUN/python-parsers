...
class Spider1Spider(scrapy.Spider):
    ...
    def parse_all_category(self, response):
        ...
        category_urls = response.xpath(
            '//li[@class="dropdown-submenu"]/a/@href').getall()
    ...
    def pagenation(self, response):
        pages_count = response.xpath(
            '//div[@class="col-pager"]/a/@href').getall()[-1].split('=')[1]
    ...
    def parse_product_links(self, response):
        ...
        for item in response.xpath(
                ...
                //div[@class="item-product-inner"]/a/@href').getall():
            yield response.follow(item, self.parse)

    def parse(self, response):
        item = FourFragItem()
        item['category_name'] = response.xpath(
            '//li[@class="dropdown-submenu"]/a/span/text()'.strip()).getall()
        item['url'] = response.url
        item['name'] = response.xpath('//h1[@itemprop="name"]/text()').getall()
        item['price'] = response.xpath(
            '//div[@class="panel-body prices product-info"]\
            //span[@class="item-price"]/text()').getall()
        item['img_link'] = response.xpath(
            '//div[@class="image-inner"]//a[@class="thumbnail"]/img/@src'
            ).get()
        item['manufacturer'] = response.xpath(
            '//div[@class="col-xs-60 blockshortinfo"]/strong/text()').get()
        item['specifications'] = response.xpath(
            '//table[@class="table table-striped table-hover"]\
            /tbody/tr/th/text()'.strip()).getall()
        yield item
