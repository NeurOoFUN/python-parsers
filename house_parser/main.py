...
class Parser:
    ...
    def __init__(self):
        self.input_url = input(
            'Введите название города: (АНГЛИЙСКИМИ БУКВАМИ)')
        ...
        if not os.path.exists('data'):
            os.mkdir('data')
        with open(f'data/CSV_table_to_{self.input_url}_.csv', 'w',
    ...
    def get_start_links(self):
        ...
        print('Скрипт запущен. Ожидайте завершения сбора данных. \
            Это займет время...')
        for item in start_links:
            all_start_links = self.domain + item.find(
                'a', class_='site-map-item__link').get('href')
            self.start_category_names = item.find(
                'a', class_='site-map-item__link').get_text().strip()
    ...
    def get_links_to_products(self, pagen_links):
        ...
        while True:
            response = self.session.get(
                url=pagen_links, params={'page': self.i}).text
            soup = BeautifulSoup(response, 'lxml')
            all_product_links_list = []
            print(f'Этап обработки: тип недвижимости -- \
                {self.start_category_names} / страница № {self.i}')
            try:
                links = soup.find('div', class_='search-content__results') \
                ...
                for item in links:
                    self.all_product_links = self.domain + \
    ...
    def all_datad_from_pcoduct(self, all_product_links):
        ...
        ad_author_block = soup.find(
            'div', class_='offer-card-contacts__wrapper _published') \
            .find('div', class_='offer-card-contacts__block')
        try:
            self.author_type = ad_author_block.find(
                'div', class_='offer-card-contacts__person _type') \
                .get_text().strip()
        except Exception:
            self.author_type = 'Тип автора объявления не указан.'
        # Имя автора объявления.
        try:
            self.author_name = ad_author_block.find(
                'span', class_='ui-kit-link__inner').get_text().strip()
        except Exception:
            self.author_name = 'Имя автора объявления не указано.'
        # Номер телефона автора объявления.
        try:
            author_phone = soup.find_all(
                'a', class_='offer-card-contacts-phones__phone')
    ...
    def save_to_csv(self):
        ...
        """
        with open(f'data/CSV_table_to_{self.input_url}_.csv', 'a',
                  encoding='utf-8') as file:
            writer = csv.writer(
                file, dialect='excel', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(
                (
                    ...
                    self.author_type,
                    str(self.all_phone_list).replace('[', '')
                                            .replace(']', '').replace("'", '')
                )
            )
    ...
    def convert_csv_to_excel(self):
        ...
        wb = Workbook()
        ws = wb.active
        with open(f'data/CSV_table_to_{self.input_url}_.csv', 'r',
                  encoding='utf-8') as file:
    ...
    def run(self):
        ...
        try:
            ...
            print('Произошла ошибка!!!\nЛибо данного города на сайте нет, либо \
                вы не правельно написали название города.\
                \nПопробуйте еще раз.')
            parser = Parser()
            parser.run()
